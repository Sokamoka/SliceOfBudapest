      ...
      <div class="holder">
        <div class="slide-wrapper" :style="positionStyle" v-show="overlayActive">
          <div class="slide" v-for="(image, index) in images" :key="index">
            <clazy-load class="tvb-img" :ratio="0.1" :src="images[index]">
              <img :src="images[index]" slot="image">
        ...
        <p v-if="caption">{{ pageIndicator.index }}/{{ pageIndicator.pages }}</p>
...
export default {
  ...
  computed: {
    ...
    currentTitle() {
      return this.images[this.currentImage]
        ? this.images[this.currentImage].title
        : '';
    }
  },
  mounted() {
  ...
  watch: {
    index(index) {
      if (index !== null) this.clickImage(index);
    }
  },
  methods: {
    initResize() {
    ...
    clickImage(index) {
      ...
      this.duration = 300;
      this.position.x =
        (this.swiperRect.left + this.swiperRect.right) * this.currentImage;
